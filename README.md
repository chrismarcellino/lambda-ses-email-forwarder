...
A Python3 script for AWS Lambda that uses the inbound/outbound capabilities of
...
used gratefully under MIT license.)
...
2. In AWS SES, verify the domains for which you want to receive and forward
...
3. If you have not configured inbound email handling, create a new Rule Set.
...
5. The S3 bucket policy needs to be configured so that your IAM user has read
...
6. Optionally set the S3 lifecycle for this bucket to delete/expire objects
after a n days to clean up the saved emails.

7. Set the environment variables to configure the script forwarding addresses etc.
